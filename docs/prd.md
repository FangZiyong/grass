...
新建表时，系统默认自动添加以下内部字段（实际字段集合可在技术方案中最终确定，本 PRD 描述期望）：
...
> 注：
>
> - 列表中展示的是「数据类型（UI 类型）」，底层数据库实际类型（db_type）对用户透明，由系统根据数据类型和关联配置自动选择。
...
| 数据类型 | 是   | 下拉选项：文本 / 整数 / 小数 / 日期 / 日期时间 / 布尔 / 关联     |
| 是否主键 | 否   | 默认 false，V1 仅支持单主键；仅当数据类型 ≠ 关联 时允许勾选      |
| 是否必填 | 否   | 映射 NOT NULL，默认 false                                        |
| 默认值   | 否   | 根据“数据类型”展示不同输入组件；**当数据类型=关联时隐藏 / 禁用** |
| 描述     | 否   | 0–200，用于说明字段含义                                          |

**生成字段编码：**

- 填写“字段名称”后自动调用“生成字段编码”接口（规则见第 11 章）；
- 同一表内必须唯一；生成失败阻止提交并提示错误。

**数据类型（UI 类型）说明：**

- 文本：用于字符串字段；系统内部选择适合的 varchar/text 类型；
- 整数：用于计数、ID 这类整型数据；
- 小数：用于金额、比率等需要小数精度的数据；
- 日期：只包含年月日；
- 日期时间：包含日期 + 时分秒；
- 布尔：是/否；
- 关联：表示**这个字段用于关联另一张表的记录**（详见 8.6.2.2 和 8.6.6）。

> 对于非“关联”类型，用户只需要选一个“数据类型”，底层具体使用什么数据库类型（例如 int32/int64、decimal(18,2)）由系统内部根据统一规则决定，不在 UI 中暴露。

##### 8.6.2.2 新增“关联”字段

当用户在“数据类型”下拉中选择「关联」，表单行为调整如下：

1. **字段级别 UI 行为**

   - “是否主键”直接禁用（关联字段不允许作为主键）；
   - “默认值”字段隐藏或禁用（关联字段不支持默认值，由是否必填决定是否允许为空）；
   - “是否必填”仍可配置：
...
新增一个「关联配置」分组，仅当数据类型=关联时显示：

| 配置项               | 必填 | 说明                                                                 |
| -------------------- | ---- | -------------------------------------------------------------------- |
| 关联表               | 是   | 从当前租户内用户可见且 `TABLE_SCHEMA ≥ VIEW` 的表列表中选择          |
| 关联字段             | 否   | 选中关联表后，默认使用该表主键字段；也可从该表字段列表中选择其他字段 |
| 展示字段（用于显示） | 否   | 决定数据页中展示关联记录时使用哪个字段的值，如用户表的“用户名”       |
| 选择方式             | 否   | 下拉选择 / 搜索选择，用于数据页录入时的交互方式                      |

3. **底层类型与兼容性（完全对用户隐藏）**

- 用户不再选择“底层字段类型”，也不用理解数值/字符串兼容；
- 系统内部逻辑：

  - 读取“关联字段”的底层存储类型 db_type；
  - 将当前字段的 db_type 自动设置为**与关联字段相同**；
  - 如未来要支持更复杂情况（例如总是用 string 存外键），由技术设计自行约定，产品不暴露。

- 若选择的关联字段本身是“关联字段”（避免嵌套关联）：

  - V1 约定：**关联字段列表中仅展示目标表的普通字段和主键字段，不包含目标表的“关联字段”**；
  - 避免用户形成“关联链条”的复杂心智。
...
- 字段成功创建后（无论普通还是关联）：
  ...
  - 若业务需要从普通字段变为关联字段，推荐流程：

    1. 新建一个关联字段；
    2. 用任务流 / SQL 把旧字段数据迁移到新字段；
    3. 视情况删除或停用旧字段。

- 保存时强制校验：

  - 数据类型=关联 → 必须选择“关联表”；
  - 未显式选择“关联字段”时，默认使用目标表主键字段；
  - 若目标表没有可用字段（极端情况），阻止保存并提示：“目标表没有可用字段，无法建立关联。”
...
- 字段编码（code）；
- 数据类型（UI 类型）：文本 / 整数 / 小数 / 日期 / 日期时间 / 布尔 / 关联；
- 底层存储类型（db_type，完全不对用户展示）；
- 是否主键；
- 是否系统字段（is_internal）。

> 说明：
>
> - 「数据类型」一旦保存即锁定，避免牵涉数据迁移；
> - 对关联字段，关联表 / 关联字段也在 V1 锁定，不支持编辑，只能通过新建字段方式迁移。
...
- 使用 `updated_at` 做简单并发控制：

  - 提交时若发现 `updated_at` 已变化，返回冲突提示，要求用户刷新后重试。

---

#### 8.6.4 删除字段（保持原逻辑 + 补充对关联字段的说明）
...
1. Relation 中是否引用该字段；
2. Flow 节点是否使用该字段；
3. Dataset / Widget 是否使用该字段；
4. RowPermission / ColumnPermission 是否引用该字段；
5. FilterDSL（各类过滤配置）中是否引用该字段；
6. 其他字段的关联配置是否使用该字段作为“关联字段或展示字段”。

只要存在任意引用 → 删除失败，并提示引用来源；
无引用时 → 删除字段元数据 + `ALTER TABLE DROP COLUMN`，并清理 ColumnPermission；
若该字段是“关联字段”，同时删除对应自动生成的 Relation 记录（见下节）。

---

#### 8.6.5 系统字段规则

系统字段（id / created_at / updated_at / created_by / updated_by）：

- UI 中的“数据类型”可统一展示为“系统字段”，不开放编辑；
- 不允许配置为“关联”类型，也不能作为其他字段的“关联字段”。

---

#### 8.6.6 关联字段（Reference 字段）行为与约束（与新设计对齐）

这里只讲“关联字段”的行为，不再出现“字段类型兼容性”的概念，全部对用户隐藏。

##### 8.6.6.1 元数据（产品层面）

在 Field 元数据中引入：

- `ui_type`：枚举，值之一为 `REFERENCE`（对应 UI 中的“关联”）；
- `is_reference`：bool，与 `ui_type=REFERENCE` 保持一致；
- `ref_table_id`：关联表 ID；
- `ref_field_id`：关联字段 ID；
- `ref_display_field_id`：展示字段 ID；
- `ref_input_mode`：SELECT / SEARCH。

底层存储类型 `db_type` = 关联字段的 db_type（内部逻辑，不写给用户看）。

##### 8.6.6.2 数据页中的表现

**查看：**

- 显示“关联展示字段”的值（如用户名），而非原始 ID；
- 找不到目标记录时，显示“（已失效）ID=xxx”之类提示；
- 若用户对目标表 `TABLE_DATA ≥ VIEW`，可提供“跳转查看详情”的入口。

**编辑：**

- 若用户对目标表 `TABLE_DATA ≥ VIEW`：
  ...
  - `ref_input_mode = SEARCH`：
    ...
    - 输入关键字 → 后端在展示字段上模糊搜索 → 返回候选列表。

- 若对目标表 `TABLE_DATA < VIEW`：
...
##### 8.6.6.3 Relation 自动同步（逻辑不变，略调 wording）

- 新建关联字段时：

  - 系统自动创建一条 Relation 记录：

    - 当前表 → 关联表；
    - 关系类型可标记为 “多对一(MANY_TO_ONE)”（内部用，UI 里可以不强调术语，只在高级配置页展示）。

- 删除关联字段时：

  - 自动删除对应 Relation（前提是没有其他地方依赖，可按“一字段一 Relation”简化）。

##### 8.6.6.4 目标表 / 目标字段删除

- 目标表被引用为 ref_table_id 时，不允许删除；
- 目标字段被引用为 ref_field_id / ref_display_field_id 时，不允许删除；
  这部分逻辑已经在 8.6.4 删除字段前的依赖检查中覆盖。

##### 8.6.6.5 权限约束（与原设计一致）

- 当前表的行/列权限 → 限制用户能否看到 / 编辑这个关联字段；
- 目标表的行权限 → 限制候选列表中能看到哪些记录；
- 目标表的列权限 → 限制展示字段和搜索字段是否可用；
- 目标表的资源级权限 → 限制是否可以通过 UI 获取候选记录。
...
- 后端删除逻辑：
  ...
  - V1 可采用物理删除；
  - 若采用软删除，需要在表结构中增加 `is_deleted` 等字段并调整所有查询逻辑（需在架构层统一设计，本 PRD 不强行要求软删除）。
...
1. 关系名称（可选，默认为“表 A.字段 A → 表 B.字段 B”）
2. 当前表的角色：
