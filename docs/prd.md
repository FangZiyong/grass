...
- 报表（Reports）：数据集、可视化查询、仪表盘；支持图表资产化、复用与组合展示；
...
- **tenant_code / role_code（编码）**：用于展示与检索的稳定编码，**由后端自动生成**，前端/调用方不需要传入；业务侧主要通过 `tenant_id / role_id` 进行关联。

#### 权限与资源

- **Role / 角色**：租户内部定义的权限集合，如“数据工程师”、“分析师”。
- **Resource / 资源**：可授权的实体，如表、任务流、数据集、仪表盘。
...
- **Dataset / 数据集**：基于某张来源表（或视图）生成的**可复用数据集表**，封装字段选择、基础过滤（base_filter）与刷新策略，用于稳定支撑下游分析与展示。
- **Chart / 图表**：在可视化查询中保存的“查询配置 + 可视化配置”，归属某个数据集，可被多个仪表盘复用（不承载布局）。
- **Dashboard / 仪表盘**：聚合多个图表实例的可视化页面，包含布局与（可选）全局筛选，支持访问、分享、导出。
- **DashboardItem / 图表实例**：仪表盘中的一个组件实例，引用某个 Chart，并附带布局信息（x/y/w/h）。
...
本平台希望通过**多租户 + 配置化建模 + 可视化任务流 + 仪表盘**的组合模式，解决以上痛点。
...
> 一款面向企业内部或 SaaS 场景的「多租户配置化数据建模与报表平台」，在统一平台上为多个组织/项目提供数据建模、ETL、权限控制和可视化仪表盘能力。

可作为：

- 企业内部数据中台 / 营销数据平台的“上层工具”；
- SaaS 产品中，为不同客户提供可定制仪表盘的“配置型分析平台”；
...
- **对业务方 / 分析师**
  ...
  - 通过 Dataset + Dashboard 自助搭建仪表盘，无需找开发；
  - 业务术语友好（字段显示名称、预设数据集）。
...
**场景 A：游戏项目的运营数据仪表盘（完整链路：建模 → ETL → 仪表盘）**
...
- 租户工作区基础框架
  ...
  - 租户选择与切换；
  - Modeling / Flows / Reports / Settings 四大模块导航。
...
- 报表模块（Reports）
  ...
  - 数据集（Datasets）：基于来源表生成数据集表，配置字段、base_filter、刷新策略；独立资源树（`scope=DATASET`），提供状态与刷新日志。
  - 可视化查询（Charts / 探索分析）：基于数据集拖拽配置生成可视化结果，并可保存为图表（Chart）用于复用与加入仪表盘；图表做列表管理（不做树）。
  - 仪表盘（Dashboards）：独立资源树（`scope=DASHBOARD`），提供编辑器拖拽布局、添加图表实例、（可选）全局筛选、分享与导出。

- 权限与 DSL（基础能力）

  - 统一过滤 JSON DSL 的语法定义；
  - 资源级权限（表结构 / 表数据 / Flow / Dataset / Dashboard）；
...
- 日/周/月活跃租户数（有登录行为或运行 Flow / 打开仪表盘）；
...
- 关键操作错误率（“仪表盘加载失败”、“Flow 配置错误”等）低于一定阈值；
...
| 报表：数据集（创建/编辑/刷新） | 管理            | 管理（可选）        | 管理         | 只读（VIEW） |
| 报表：图表（创建/编辑/删除）   | 管理            | 视数据集权限        | 管理         | 只读（可见） |
| 报表：仪表盘（创建/编辑/删除） | 管理            | 视权限可管理        | 管理         | 只读（VIEW） |
| 报表：查看（访问边界）         | 全部            | 全部                | 全部         | 仅授权仪表盘（且需关联数据集可见） |
...
4. **跨租户复用能力（仅配置级）**
   ...
   - 允许在产品设计上保留“模板导出/导入”的概念，以便在后续版本支持：
     ...
     - 在一个租户中导出模型/Flow/Dashboard 模板，在另一个租户中导入。
...
- 用于表示租户内“可分层管理”的资源目录结构（Folder + Resource）。
...
- 节点类型：
  ...
  - FOLDER：目录；
  - TABLE / FLOW / DATASET / DASHBOARD：具体资源。
...
#### 3.1.10 Dataset / Chart / Dashboard

**Dataset / 数据集**

- 基于某张来源表（或视图）生成的可复用数据集表；
- 核心配置：字段选择、base_filter（FilterDSL）、刷新策略；
- 下游（可视化查询、图表、仪表盘）只读取数据集表，不直接读取来源表。

**Chart / 图表**

- 可视化查询中保存的资产，归属某个数据集；
- 包含：`query_config`（维度/指标/过滤/排序/limit 等）与 `viz_config`（图表类型与样式）；
- 不包含布局；布局属于仪表盘内的图表实例（DashboardItem）。

**Dashboard / 仪表盘**

- 聚合多个图表实例的页面；
- 包含：布局（layout_json）与（可选）全局筛选（filters_json）；
- 图表实例（DashboardItem）引用 Chart，并携带 `x/y/w/h` 等布局信息。
...
1. **主键类型**
   ...
   - 所有核心业务表（GlobalUser、Tenant、TenantUser、Table、Field、Flow、Dataset、Chart、Dashboard、DashboardItem 等）的主键 `id` 统一采用：
...
2. **tenant_id 使用规范**
   ...
   - 所有租户内实体表（Table、Field、Flow、Dataset、Dashboard、ResourceTree、Role、Permission 等）必须包含 `tenant_id`；
...
- Table.code；
- Field.code；
- 未来可能扩展的 Dataset.code、Chart.code、Dashboard.code 等。
...
后续所有模块（建模、任务流、数据集&仪表盘、设置）涉及到“筛选 / 查询 / 权限”时，都应引用本章约定，避免各自实现。
...
- 可视化查询/图表（Chart）的过滤条件（维度/指标过滤）；
- 仪表盘（Dashboard）的全局筛选（filters_json，可选）；
- Flow 中的“过滤节点”。
...
对于任意一张表的查询（表数据页、Flow 节点查询、Dataset/Chart 查询）统一约定：

1. 应用 Dataset.base_filter（若存在）；
2. 应用 Chart / Flow 节点设置的业务过滤条件；
...
- 对 HIDDEN 字段：
  ...
  - 查询接口不返回该字段；
  - 表数据页、Chart 字段选择器中不展示；
  - 若历史配置中某 Chart 已引用该字段，而当前用户对该字段为 HIDDEN：

    - 查询该 Chart 时，后端可直接返回字段权限错误，前端提示“当前无权使用字段 X”。
...
系统通过 ResourceTree 组织资源（表、Flow、Dataset、Dashboard），包含 Folder 节点和资源节点：

- Folder 节点上可以配置“默认权限”；
- 对于该 Folder 下**未单独配置权限**的子资源节点（表 / Flow / Dataset / Dashboard），使用最近祖先 Folder 的默认权限；
...
#### 6.3.3 报表模块（Empty States）

- 当租户内尚未创建任何数据集/图表/仪表盘时，报表模块首页必须展示引导文案：

  > “暂时还没有报表资产。建议先从常用业务表创建数据集，再在可视化查询中保存图表，最后将图表加入仪表盘进行日常查看与分享。”

- 按权限展示快捷入口：

  - 若用户在任一数据集目录具备 `DATASET >= EDIT`：显示“新建数据集”；
  - 若用户在任一仪表盘目录具备 `DASHBOARD >= EDIT`：显示“新建仪表盘”；

- 必须提供“推荐流程”提示（不超过 1 行）：

  > “创建数据集 → 可视化查询保存图表 → 创建仪表盘并添加图表”
...
此处配置的是：**角色**在**表 / 任务流 / 数据集 / 仪表盘**上的 NONE / VIEW / EDIT / MANAGE 权限。
...
- 展示 `scope=DATASET` 的 ResourceTree（Folder + Dataset）；
- 对每个 Dataset 节点配置：

| 权限项       | 可选值                       |
| ------------ | ---------------------------- |
| Dataset 权限 | NONE / VIEW / EDIT / MANAGE |

Folder 节点支持“默认 Dataset 权限”。

#### 7.3.5 仪表盘权限配置

在“仪表盘权限” Tab 内：

- 展示 `scope=DASHBOARD` 的 ResourceTree（Folder + Dashboard）；
- 对每个 Dashboard 节点配置：

| 权限项          | 可选值                       |
| --------------- | ---------------------------- |
| Dashboard 权限 | NONE / VIEW / EDIT / MANAGE |

Folder 节点支持“默认 Dashboard 权限”。
...
- 资源权限：
  ...
  - 修改角色在表 / Flow / Dataset / Dashboard 上的资源级权限。
...
- 操作时间；
- 操作类型（枚举）；
- 操作对象（角色 ID / 用户 ID / 表 ID / Flow ID / Dataset ID / Dashboard ID 等）；
- 变更摘要（如“字段 amount 列权限：READWRITE → HIDDEN”）；
- 操作结果（成功 / 失败及失败原因）。

前端提供“审计日志”查询页面（只读），用于查看权限变更记录，详见第 13 章。
...
4. **可供下游复用的元数据中心**
   ...
   - 为任务流（Flows）、报表（数据集 / 图表 / 仪表盘）（Reports）提供统一的元数据来源：
...
4. 分析师在“数据集模块”中基于该表创建 Dataset，再配置仪表盘展示订单分析指标。
...
3. 是否被任何报表资产依赖（通过 Dataset 间接使用，如图表/仪表盘）；
4. 是否被其他表的“关联字段”引用：
...
- 返回结构化错误信息，至少包括：
  ...
  - 引用类型（Flow / Dataset / Dashboard / 关联字段 / 权限）；
  - 数量；
...
2. 是否被任何 Flow 节点使用；
3. 是否被任何 Dataset 使用（图表/仪表盘通过 Dataset 间接依赖）；
...
> 本节只规定“必须记录哪些日志”及其内容结构，具体查询展示页面在第 13 章定义。
...
#### 8.8.3 审计查询（与第 13 章联动）

- 本章仅要求“必须记录”；
- 具体展示方式交由第 13 章统一设计，例如：

  - 在租户 Settings 中提供“建模操作日志”页面：

    - 支持按时间、操作人、操作类型筛选；

  - （后续版本）在平台后台提供跨租户审计能力（本期不提供 UI，仅预留查询能力）：

    - 供平台管理员查看高风险操作（如批量删表等）。
...
3. **安全可控的数据读写**
   ...
   - 所有节点的读写行为严格受资源级权限约束；
   - 流程执行时对全表进行处理，不叠加 Row/Column 权限的限制（Row/Column 主要作用于交互式查询与仪表盘）。
...
**定位**
数据集（Dataset）是报表模块的“数据资产层”，负责将建模模块中的业务表数据，以“可分析口径 + 可复用范围”的方式沉淀下来，供 **可视化查询（第 11 章）** 与 **仪表盘（第 12 章）** 复用。

**本期必须达成的目标**

1. 用户能够基于某张建模表创建数据集，并将数据集同步到分析存储（如 Doris/ClickHouse）形成独立的数据表（后续查询均基于该数据集表进行）。
2. 数据集创建时固定其可同步范围（行/列条件快照），避免后续权限变化导致“隐式扩权/口径漂移”。
3. 支持数据集刷新（首次/手动/定时）与刷新日志，且失败原因对用户可理解、可定位。
4. 数据集、仪表盘两类资源均采用树状目录管理且**不共用同一棵资源树**，按资源树授权（你已明确）。

**不在本期范围（Out of Scope）**

- 用户自写 SQL 直接访问建模表或外部数据源
- 多表 join 形成数据集（本期仅支持单表；多表由 Flow 先产出宽表）
- 数据集版本管理/多快照回溯
- 跨租户共享数据集

---

### 10.2 核心概念与数据范围（Scope Snapshot）

现有 PRD 中 Dataset 仅描述为“基于表的可复用数据视图 + base_filter” ，本章对 Dataset 做**升级定义**（产品语义）：

**Dataset = 元数据配置 + 物理落库表**

- **元数据配置**：来源表、字段集合、基础过滤（base_filter）、刷新策略、Owner、目录归属等。
- **物理落库表**：用于分析查询的目标表（查询只访问该数据集表）。

**Scope Snapshot（范围快照）——创建即固化**
创建数据集时系统必须固化以下内容，后续刷新与查询均以此为准：

1. **允许列集合（Allowed Columns）**：创建者在来源表上“可见的字段集合”，与列权限（ColumnPermission）一致（HIDDEN 不可进入集合；READONLY/READWRITE 可进入集合，见现列权限定义 ）。
2. **行过滤规则（Row Scope Filter）**：创建者在来源表上的行权限合并结果（RowPermission 的 OR 合并规则在现文档已有约定 ，本章引用该合并结果作为数据集行范围快照）。
3. **数据集基础过滤（Dataset Base Filter）**：用户在创建数据集时额外配置的 base_filter_json（FilterDSL，现文档已将其纳入统一 DSL 使用场景 ）。
4. **最终范围表达式**：
   `FinalScope = RowScopeFilter AND DatasetBaseFilter`
   （RowScopeFilter 为空时视为 TRUE；DatasetBaseFilter 为空时视为 TRUE）

> 规则要求：数据集的范围只能“显式修改”，不得因创建者权限扩大而自动扩大；也不得因权限缩小而悄悄改变既有范围（而是以“刷新合规校验”来阻断，见 10.6）。

---

### 10.3 信息架构与导航入口

> 租户工作区原导航为 Modeling / Flows / Boards / Settings 。本期将“Boards（报表）模块”升级为三页：**数据集 / 可视化查询 / 仪表盘**（三章分别详述）。

**数据集页面结构**

- 左侧：数据集资源树（Folder + Dataset）
- 右侧：列表区 / 详情区（依据当前选中节点切换）
...
- 状态（见 10.5）
- 来源表（Table）
- 最近一次成功刷新时间、最近一次刷新耗时、最近一次失败原因摘要
- 引用数（被多少 Chart 引用、被多少仪表盘引用）
- 更新时间、创建时间

**全局交互规范（数据集模块内）**

- 所有“创建/编辑/删除/授权/转移 Owner/刷新”操作必须有明确成功与失败反馈，失败原因可读（遵循你现 PRD 的错误反馈原则：业务约束需给出关键依赖信息 ）。

---

### 10.4 权限模型（Dataset）

现文档 RolePermission 覆盖 TABLE/ FLOW/ BOARD，并统一权限等级 NONE/VIEW/EDIT/MANAGE 。本期报表模块要求扩展为：

**资源树 Scope（新增）**

- `scope=DATASET`：数据集资源树（Folder + Dataset）
- `scope=DASHBOARD`：仪表盘资源树（Folder + Dashboard）
  （原 scope=BOARD 将升级为 scope=DASHBOARD；数据集与仪表盘两棵树独立）

**RolePermission 资源类型（新增）**

- `DATASET`
- `DASHBOARD`
  （原 `BOARD` 迁移为 `DASHBOARD`；历史数据迁移规则在技术方案中定义）

**Dataset 权限等级与能力（必须严格按按钮收敛）**

| 权限   | 能做什么                                                                                                              |
| ------ | --------------------------------------------------------------------------------------------------------------------- |
| NONE   | 不可见（资源树不展示）                                                    |
| VIEW   | 查看数据集详情；在可视化查询中选择并基于其数据进行分析；允许查看其图表列表与引用关系；允许查看刷新日志                |
| EDIT   | 新建/编辑数据集配置（字段选择、base_filter、刷新策略）；手动触发刷新；查看更详细的刷新日志（如 SQL 摘要不在本章要求） |
| MANAGE | 授权与目录管理（移动/重命名目录）；转移 Owner；删除数据集；强制暂停/恢复刷新                                          |

**Owner 的产品语义**

- 数据集创建人默认 Owner（Owner 是“管理责任人”，不等于“可以绕过权限的人”）。
- Owner 的权限由其角色权限合并结果决定（多角色合并规则沿用系统统一规则）。
- Owner 发生变更必须写入审计日志（谁在何时将 Owner 从 A 转给 B，原因可选填）。

---

### 10.5 数据集状态机与展示

数据集必须维护清晰状态，并在列表与详情页可视化展示：

**状态枚举（必须实现）**

1. `DRAFT`（可选）：仅保存配置尚未首次刷新（若你不需要草稿，可直接用 ACTIVE+未刷新标识）
2. `ACTIVE`：可正常刷新、可被查询
3. `REFRESHING`：刷新进行中（同一数据集禁止并发刷新）
4. `FAILED`：刷新失败（配置错误、源表不可用、超时等）
5. `BLOCKED`：刷新被阻断（Owner 合规校验不通过，详见 10.6）
6. `PAUSED`：被管理员/Owner 主动暂停刷新

**状态展示规范**

- 列表：状态 Tag + 悬浮提示（最近一次失败原因摘要、最近成功时间）
- 详情：状态面板必须包含

  - 状态说明
  - 最近一次刷新记录（开始时间、结束时间、耗时、结果）
  - 明确的“下一步操作”按钮（例如 FAILED 提供“重试刷新”；BLOCKED 提供“查看原因/转移 Owner/调整范围”等）

---

### 10.6 创建与编辑（向导式）——字段、校验、确认

#### 10.6.1 新建数据集入口

- 入口 A：数据集模块空状态/列表页主按钮「新建数据集」
- 入口 B：来源表详情页（Modeling）提供快捷入口「基于该表新建数据集」

#### 10.6.2 创建向导步骤（必须）

**Step 1：选择来源表**

- 可选范围：用户对该表具备 `TABLE_DATA >= VIEW` 的表（资源树只列出可见表）
- 必填项：来源表
- 校验：

  - 若无任何可选表：提示“暂无可用来源表，请联系管理员授予表数据查看权限”

**Step 2：选择字段（Allowed Columns）**

- 左侧字段列表：只展示创建者列权限非 HIDDEN 的字段（HIDDEN 必须完全不可见 ）
- 选择方式：勾选字段；支持全选/搜索/按类型筛选
- 必填校验：至少选择 1 个字段
- 约束：

  - 若字段后续被建模修改/删除：需触发影响提示与刷新失败（见 10.9）

**Step 3：配置数据集基础过滤（Dataset Base Filter）**

- 使用统一 FilterDSL 可视化条件编辑器
- 校验：

  - 字段类型与操作符匹配（例如 between 必须是数值/日期）
  - value 格式正确（错误提示必须指出字段名与原因，沿用现错误规范 ）

**Step 4：刷新策略**

- **刷新模式（V1 固定）**：仅全量刷新（FULL）
- **定时刷新（V1 固定）**：支持 CRON 表达式（默认关闭）

  - 支持预置：每小时/每日/每周（选择预置会自动生成 cron）
  - 支持自定义 cron（需通过后端校验）
  - 最小间隔：5 分钟
  - 需展示：下次触发时间

- 必须支持“手动刷新”按钮（受权限控制，见 10.7）


**Step 5：确认与创建**

- 展示 Scope Snapshot 汇总（必须对用户可读）：

  - 选择字段数量与字段清单
  - RowScopeFilter 是否存在（如存在，提示“该数据集将继承你在来源表上的行权限范围”）
  - base_filter 内容摘要
  - 最终范围表达式：RowScopeFilter AND base_filter
...
- 保存前必须弹出“影响分析”（见 10.9），并要求用户确认。

---

### 10.7 刷新与合规校验（Owner 资格校验）

你已确定“改用系统角色跑”，本 PRD 仅描述产品规则：**刷新由系统统一执行，但刷新是否允许，取决于 Owner 是否仍满足固化范围的资格**。

#### 10.7.1 刷新触发方式

> **V1 固定：仅支持全量刷新（FULL refresh）**。不支持增量刷新、CDC、按分区增量等能力。

**触发来源**

1. **首次刷新（创建后）**
   - 创建向导完成后，进入“确认页”引导用户点击“立即刷新并生成数据集表”；
   - 首次刷新成功后，数据集状态进入 `READY`；失败进入 `FAILED`（见 10.7.3）。

2. **手动刷新（Owner/有权限用户）**
   - 数据集详情页提供“立即刷新”按钮；
   - 权限：`DATASET >= EDIT` 才可触发；
   - 行为：触发一次全量刷新任务，生成新版本数据（覆盖数据集表当前内容）。

3. **定时刷新（CRON）**
   - 数据集详情页提供“定时刷新”配置区域（默认关闭）；
   - 权限：`DATASET >= EDIT` 才可配置/启停；
   - 配置方式：以 **cron 表达式** 形式保存（支持预置：每小时/每日/每周等）。
   - 运行时区：以 **租户时区** 为准（如租户未配置，则使用系统默认时区）。

**cron 校验规则（必须后端强校验）**

- 语法必须合法；非法直接拒绝保存并返回错误提示；
- 允许的最小间隔：**5 分钟**（避免误配置造成任务风暴）；
- 单个数据集最多 1 条有效 cron；启用时必须能计算出下一次触发时间；
- 展示：配置成功后，前端需展示“下次触发时间/最近 3 次触发记录”。
...
1. **配置可用性校验**
   ...
   - 来源表仍存在
   - 选择字段仍存在且类型兼容
   - base_filter 可解析（FilterDSL 有效）

2. **Owner 合规校验（关键）**

   - Owner 在来源表上仍具备覆盖 Allowed Columns 的列权限（不可出现“快照中包含的列，Owner 现在变为 HIDDEN”）
   - Owner 在来源表上的行权限合并结果仍能覆盖 RowScopeFilter 快照（Owner 权限收紧导致无法覆盖时视为不通过）

3. **通过则允许刷新；不通过则阻断**

#### 10.7.3 阻断（BLOCKED）行为（必须明确闭环）

当 Owner 合规校验不通过：

- 数据集状态置为 `BLOCKED`
- 刷新记录写入失败（失败类型=OwnerIneligible）
- UI 必须提供三种修复路径（具备权限者可操作）：

  1. **转移 Owner**（Dataset MANAGE）
  2. **缩小范围**（Dataset EDIT：减少字段/收紧 base_filter，使其重新满足）
  3. **暂停刷新**（Dataset MANAGE，可选，但若提供则必须写清）

> 重要约束：BLOCKED 期间不允许自动重试刷新（避免刷屏与误报）；只有当用户完成修复动作后才允许再次触发刷新。

---

### 10.8 数据集被下游使用时的规则（与第 11/12 章联动）

**数据集对下游的稳定承诺**

- 下游（Chart、Dashboard）只能读取数据集表，不读取来源表。
- 数据集字段/过滤变更会影响下游展示，必须在保存时提示影响范围（见 10.9）。

**数据集权限对下游的约束**

- 用户在可视化查询或仪表盘中访问数据集数据时，必须同时满足其对数据集的权限要求（详细联合规则在第 12 章定义）。

---

### 10.9 影响分析、引用保护与删除规则

现错误规范要求“删除存在依赖时提示关键依赖信息” ，数据集必须做到：

#### 10.9.1 引用关系展示（详情页必须提供）

- “被引用的图表”：列表（图表名、创建人、最近更新时间、所属仪表盘数）
- “被引用的仪表盘”：列表（仪表盘名、Owner、最近访问时间）
- “引用统计”：引用次数、最近 7 天使用次数（可选）
...
- 若数据集被任何图表/仪表盘引用：
  ...
  - 默认不允许删除
  - 提示文案必须列出关键依赖（至少列出前 N 个，可分页）
  - 提供“去解除引用”跳转入口

- 仅当无引用时允许删除（MANAGE 权限）

#### 10.9.3 编辑影响分析（必须）

当用户保存以下变更时，必须弹出影响分析并要求确认：

- 删除字段 / 字段重命名（会导致下游字段引用失效）
- 修改 base_filter（会改变数据口径）
- 修改刷新策略（会影响数据新鲜度）
...
- 受影响图表数量 + 列表入口
- 受影响仪表盘数量 + 列表入口
- 明确后果提示（例如“部分图表将无法渲染，需要重新选择字段”）

---

### 10.10 空状态与新手引导（数据集）

你现 PRD 对 Boards 空状态已提示“建议先创建数据集再配置仪表盘” ，本期将其落到数据集模块：

- 无数据集时：

  - 文案：说明数据集作用 + 建议从常用业务表开始
  - 若用户有创建权限：显示主按钮「新建数据集」
  - 若无创建权限：提示联系管理员授权
...
1. **创建-刷新-可用**：基于来源表创建数据集 → 首次刷新成功 → 可在第 11 章选择该数据集并出图。
2. **Owner 权限收紧阻断**：Owner 行/列权限收紧 → 定时刷新触发 → 数据集进入 BLOCKED，原因可见 → 转移 Owner 后恢复刷新。
3. **编辑影响分析**：删除字段/修改过滤 → 保存时明确提示影响 → 下游表现与提示一致。
4. **引用保护**：被引用的数据集无法删除，提示列出引用对象并可跳转解除引用。

---

## 11. 可视化查询（Charts / 探索分析）

### 11.1 模块定位与目标

可视化查询是“分析工作台”，用户基于某个数据集，通过拖拽式配置生成可视化结果，并可选择**保存为图表（Chart）**以供复用与加入仪表盘。

**本期必须达成的目标**

1. 基于数据集进行交互式分析（维度/指标/过滤/排序/limit）
2. 支持多种图表类型：折线、柱状、散点、表格、指标卡
3. 图表可保存、可复用、可加入仪表盘
4. 权限一致：无数据集权限无法分析；有权限则能得到稳定结果
...
- 归属数据集：`dataset_id`
- 查询配置：`query_config`（指标、维度、过滤、排序、limit、时间粒度等）
- 可视化配置：`viz_config`（图表类型、样式、格式化等）
- 元信息：名称、描述、创建人、更新时间

> 布局信息不属于 Chart；布局属于仪表盘内的“图表实例”，见第 12 章。
...
- 入口 A：报表 → 可视化查询（进入后先选择数据集）
- 入口 B：数据集详情页 → “去分析”（默认带入该数据集）

#### 11.3.2 页面区域划分（必须）

1. **数据集选择区**

   - 展示当前数据集名称、Owner、最后成功刷新时间、数据集状态
   - 支持切换数据集（需二次确认：切换将清空当前未保存配置）

2. **字段面板**

   - 字段列表按类型分组：时间/维度/指标（时间字段需自动识别：DATE/DATETIME）
   - 支持搜索、收藏字段（可选）、显示字段说明
   - 仅展示数据集中存在的字段（数据集字段是固定集合）

3. **配置面板**

   - 维度（Dimensions）
   - 指标（Metrics）
   - 过滤（Filters：使用统一 FilterDSL 编辑器）
   - 排序（Order By）
   - Limit（默认 1000，上限由全局约束给出）
   - 时间粒度（当维度含时间字段时启用：日/周/月/小时等，V1 给定固定枚举）
...
4. **预览区**
   ...
   - 加载态：骨架屏/加载提示
   - 空态：提示用户选择维度/指标
   - 错误态：字段缺失/数据集不可用/权限不足等（提示必须可读、可操作）

5. **操作区**

   - “运行/刷新预览”
   - “保存为图表”
   - “加入仪表盘”（可选：保存后才能加入，规则必须统一）

#### 11.3.3 默认行为（必须固定）

- 初次进入：不自动跑查询，提示选择维度/指标
- 任一配置变更后：不自动跑查询（避免频繁触发）；用户点击“运行”才更新预览
- 运行失败：保留用户配置，不清空

---

### 11.4 查询配置（query_config）规范

`query_config` 用于描述“从数据集表中如何取数”，**只包含查询语义**（维度/指标/过滤/排序/limit），不包含可视化样式。

- 存储位置：Chart 表 `query_config`（JSON）
- V1 约束：后端强校验 + 版本字段（便于后续兼容）
- 最大长度：**64 KB**（超出拒绝保存并提示）

#### 11.4.1 JSON Schema（V1）
...
```json
{
  ...
  "dataset_id": "ds_xxx",
  "dimensions": [
    {"field": "country", "alias": "国家"}
  ],
  "metrics": [
    {"agg": "sum", "field": "revenue", "alias": "收入"}
  ],
  "filters": [
    {"op": "between", "field": "dt", "value": ["2025-01-01", "2025-12-31"]},
    {"op": "in", "field": "game", "value": ["A", "B"]}
  ],
  "group_by": ["country"],
  "order_by": [{"by": "收入", "direction": "desc"}],
  "limit": 1000
}
```

> 说明：`order_by.by` 允许引用 `dimensions.alias` 或 `metrics.alias`；后端在保存时需进行“可解析性校验”。

#### 11.4.2 字段与校验规则（必须统一）

1. **dataset_id**
   - 必填；必须为用户可见且有 `DATASET >= VIEW` 的数据集；
   - 数据集被删除/阻断：图表进入错误态（见 11.7、12.7）。

2. **dimensions**
   - 可选；数组元素 `{field, alias}`；
   - `field` 必须存在于数据集字段清单中，且用户对该字段具备列权限（否则保存失败）；
   - 维度数上限：**20**。

3. **metrics**
   - 必填（至少 1 个）；数组元素 `{agg, field, alias}`；
   - `agg` 枚举：`count | count_distinct | sum | avg | min | max`（V1 固定）；
   - 非数值字段禁止 `sum/avg/min/max`；
   - 指标数上限：**20**。

4. **filters**
   - 可选；数组元素 `{op, field, value}`；
   - `op` 枚举（V1 最小集）：`eq | ne | in | not_in | between | gte | lte | contains`；
   - `between` 必须为长度=2 的数组；
   - 过滤条数上限：**50**；
   - 与数据集 base_filter/行权限合并：见 10.5、11.3.3（最终查询必须是 AND）。

5. **group_by**
   - 可选；仅允许引用 `dimensions.field`；
   - 若存在 `dimensions`，默认 `group_by = all dimensions`（前端可显式写入）。

6. **order_by**
   - 可选；数组元素 `{by, direction}`；
   - `direction` 枚举：`asc | desc`；
   - 排序条数上限：**5**。

7. **limit**
   - 可选；默认 **1000**；
   - 上限固定 **10000**（超过则拒绝保存并提示“已超过最大行数 10000”）。

#### 11.4.3 失败提示（统一文案）

- 字段不存在/无列权限：`“字段不可用，请重新选择字段。”`
- 过滤条件非法：`“筛选条件不合法，请检查输入。”`
- 超出上限：`“已超过最大限制：{具体限制}。”`

---

### 11.5 可视化配置（viz_config）与图表类型（必须）

`viz_config` 用于描述“如何展示 query_config 的结果”，包含图表类型与样式最小集。

- 存储位置：Chart 表 `viz_config`（JSON）
- 最大长度：**64 KB**（超出拒绝保存并提示）
- V1 原则：样式能力“够用即可”，避免引入过多自由度导致治理困难
...
```json
{
  ...
  "options": {
    "x": {"by": "dt"},
    "y": [{"by": "收入"}],
    "series": {"by": "country", "optional": true},
    "stack": false
  }
}
```
...
- `type` 必须为 V1 支持的枚举；
- `title` 可选；最大 60 字符；
- `options` 中引用的 `by` 必须能在 `query_config` 的输出列中解析到（维度/指标别名）；
- `Line/Bar` 至少 1 条 y；`Pie` 必须同时具备 category 与 value；`Metric` 必须有 value；
- 超出能力范围的样式字段（未知字段）：
  - V1 策略：**拒绝保存**（避免前后端行为不一致）。

#### 11.5.4 格式化规则（必须统一）

- 数字：千分位；保留小数位策略由前端控件统一（默认 2 位）
- 百分比：`xx.xx%`
- 时间：
  - date：YYYY-MM-DD
  - datetime：YYYY-MM-DD HH:mm:ss
- 空值：显示为 “-”，且不参与聚合（聚合口径以引擎规则为准，V1 固定为“空值忽略”）

---

### 11.6 图表资产化（Chart 列表，不做树）

你已明确 Chart 做简单列表，不做树状结构。
...
- 支持搜索：按名称/描述
- 列表字段：名称、类型、所属数据集、创建人、更新时间、被引用仪表盘数

#### 11.6.2 图表详情/操作

- 查看：预览 + 配置只读展示
- 编辑：进入工作台加载配置
- 复制：复制出一个新图表（新图表 owner=当前用户）
- 删除：受权限控制（见 11.7）
...
- 用户对目标数据集必须具备 `DATASET >= VIEW`；
- 若仅有 `DASHBOARD >= VIEW` 但缺少数据集权限：在仪表盘内该图表显示“无权限占位卡”（见 12.7），不可进入分析页。

**图表可见性**

- 用户可见的图表 = 其对图表所属数据集具备 `VIEW` 的图表。

**图表编辑与删除（V1 固定）**

- 编辑图表：满足以下任一条件

  - 图表创建人 且 对数据集具备 `VIEW`（可编辑自己创建的图表）
  - 或 对数据集具备 `DATASET >= MANAGE`（数据管理员可接管修改）

- 删除图表：**仅 `DATASET >= MANAGE`**  
  - 删除入口仅对满足条件的用户展示；
  - 删除前必须二次确认，且提示“删除将影响引用该图表的仪表盘”。

**字段权限变化对图表的影响**

- 数据集字段被移除：引用该字段的图表进入错误态，提示“字段已不存在，请编辑图表重新选择字段”。

---

### 11.8 加入仪表盘（与第 12 章联动）

- “加入仪表盘”必须选择目标仪表盘
- 加入后形成仪表盘内的图表实例（仅负责布局与局部标题等，详见第 12 章）
- 若用户无目标仪表盘 EDIT 权限：按钮置灰并提示原因
...
- 导出数据：CSV（V1 固定 CSV）
- 导出图片：PNG
- 导出权限：用户需对数据集具备 VIEW，且对图表可见

---

### 11.10 验收标准（可视化查询）

1. 有数据集 VIEW 权限的用户可进入分析、配置并成功预览图表。
2. 图表保存后在列表可检索；再次打开能恢复配置。
3. 图表加入仪表盘后展示一致；图表配置更新后仪表盘展示同步更新（若采用“引用式更新”，必须验证）。
4. 无数据集权限时不可运行查询，提示准确。
...
仪表盘是面向业务“高频消费”的可视化页面，聚合多个图表并提供刷新、分享等能力。

**本期必须达成的目标（V1 固定）**

1. 仪表盘树状目录管理（独立资源树）
2. 仪表盘编辑器：拖拽布局、添加图表、局部标题/备注（不含全局筛选）
3. 权限与分享：访问仪表盘必须同时满足仪表盘权限与其依赖数据集权限（联合校验）
4. 异常降级：缺权限/数据集不可用时只影响相关图表，不影响整盘进入
5. 自动刷新：支持按固定间隔自动刷新（见 12.4.1）

**本期明确不做**

- 全局筛选（filters_json 固定为空）
- 整盘导出
...
本系统为 0-1 新建，不存在历史存量迁移问题。V1 起即采用独立的仪表盘资源树：

- `scope=DASHBOARD` 的 ResourceTree（Folder/Item）
- Folder：仅用于组织仪表盘
- Item：仪表盘实体（Dashboard）

**页面入口**

- 租户工作区左侧导航：报表模块 → 仪表盘
- 默认进入：仪表盘列表/树（左侧树 + 右侧列表）
...
- 图表数
- 依赖数据集数
- 状态摘要（是否存在不可用图表：缺权限/数据集阻断/刷新失败）

---

### 12.3 仪表盘核心数据结构（产品语义）

#### 12.3.1 Dashboard（仪表盘）

- 基本信息：`name` / `desc` / `owner_user_id`
- 资源树归属：`resource_tree_node_id`（挂在 `scope=DASHBOARD`）
- 布局：`layout_json`
- 全局筛选：`filters_json`（**V1 固定为空/不启用**）
- 刷新：`auto_refresh`（是否开启）与 `auto_refresh_interval_min`（间隔分钟数）

> **V1 固定：引用式更新（实时更新）**  
> 仪表盘不复制 Chart 的 `query_config`/`viz_config`，仅保存对 Chart 的引用；渲染/刷新时始终读取 Chart 的最新配置。

#### 12.3.2 DashboardItem（仪表盘内的图表实例）

- `dashboard_item_id`
- `dashboard_id`
- `chart_id`（引用）
- 可覆盖信息（V1 最小集）：
  - `title_override`（可选，默认使用 Chart.title）
  - `note`（可选，作为业务说明，不影响取数）
- 位置与尺寸不在 item 表内存储，统一写入 `layout_json`（便于一次性保存布局）

---

#### 12.3.3 layout_json（V1 Schema + 校验）

- 存储字段：Dashboard 表 `layout_json`（JSON）
- 最大长度：**256 KB**
- 最大图表实例数：**50**
- 网格系统（V1 固定）：
  - `cols = 24`
  - `row_height = 12px`（前端实现细节，但口径需固定）
  - 坐标从 0 开始
...
```json
{
  ...
  "cols": 24,
  "items": [
    {"id": "di_xxx", "x": 0, "y": 0, "w": 12, "h": 10},
    {"id": "di_yyy", "x": 12, "y": 0, "w": 12, "h": 10}
  ]
}
```
...
- `items[].id` 必须是该仪表盘下真实存在的 `dashboard_item_id`
- `x/y/w/h` 必须为整数；`w/h` 必须 > 0
- `x + w <= cols`
- `items` 不允许出现重复 id
- 若布局保存失败，前端提示：`“布局保存失败，请稍后重试。”`

---

#### 12.3.4 filters_json（V1 约束）

- 存储字段：Dashboard 表 `filters_json`（JSON）
- **V1 固定：不支持全局筛选**，因此：
  - `filters_json` 必须为 `null` 或空数组 `[]`（后端强校验）
  - 前端不展示“全局筛选”相关入口与编辑 UI
...
- 打开仪表盘：按仪表盘内图表实例逐个加载渲染（先框架/骨架屏，后内容）
- 加载顺序：按 `layout_json.items` 顺序；单个图表失败不阻断整盘

**手动刷新**

- 顶部提供“刷新”按钮：
  - 行为：对所有图表实例重新拉取数据（并重新读取 Chart 最新配置，确保引用式更新生效）
  - 刷新过程展示全局 loading（不遮挡整页操作），每个图表单独显示刷新状态

**自动刷新（V1 支持）**

- 默认关闭；由 `DASHBOARD >= EDIT` 用户在查看态或编辑态开启
- 刷新间隔（V1 固定枚举）：`1 / 5 / 15 / 60` 分钟（上限固定 60）
- 行为：
  - 到点后触发“等价于手动刷新”的刷新逻辑；
  - 若用户切到后台标签页，可降频或暂停（前端实现策略可自行选择，但需保证不会无限刷请求）
- 关闭条件：
  - 用户关闭自动刷新
  - 仪表盘不可见/用户退出页面
...
- 影响：
  - 仪表盘查看态不展示筛选条；
  - 仪表盘编辑态不提供“新增筛选器/绑定图表”等能力；
  - `filters_json` 固定为空（见 12.3.4）。
...
- 栅格布局：拖拽移动、拖拽缩放、吸附对齐线
- 复制/删除图表实例
- 不支持撤销/重做（V1 不做）

#### 12.5.2 添加图表

- 从图表库选择（支持搜索、按数据集筛选、按类型筛选）
- 添加后默认布局：

  - 新增图表放到仪表盘底部第一可用位置
  - 默认尺寸（必须固定：例如 w=6,h=4）

#### 12.5.3 配置全局筛选器

- **V1 固定：不支持全局筛选器配置**（入口隐藏）。
- `filters_json` 固定为空（见 12.3.4）。
...
#### 12.6.1 Dashboard 权限等级

沿用 NONE/VIEW/EDIT/MANAGE（与现 BOARD 权限体系一致 ），定义如下：

| 权限   | 能做什么                                |
| ------ | --------------------------------------- |
| NONE   | 不可见或不可进入                        |
| VIEW   | 查看仪表盘、使用筛选、导出              |
| EDIT   | 编辑布局、添加/删除图表实例、配置筛选器 |
| MANAGE | 授权、转移 Owner、删除、移动目录        |
...
- 对于缺权限的数据集相关图表：
  - 图表位展示“缺少数据集权限”占位卡；
  - 占位卡仅提供“联系管理员授权”的指引（V1 不提供申请入口，见 12.6.4）。
- 其他图表正常展示。
...
- 成员授权（在仪表盘详情/权限面板中添加成员或角色并赋权）
  （链接分享若会引入匿名访问风险，本期不做）

#### 12.6.4 权限提示联动（V1 被动授权）

- **V1 固定：不提供“申请权限”功能**，仅支持管理员被动授权。

当用户打开仪表盘时，如遇到缺失权限：

- 仪表盘仍可进入（前提是对仪表盘具备 `DASHBOARD >= VIEW`）；
- 对于无权限的图表实例，展示“无权限占位卡”，并在卡片内展示：
  - 缺失的权限类型：数据集 VIEW / 字段权限不足（若可判定）
  - 需要访问的资源名称（数据集/图表名称）
  - 指引文案：`“如需查看，请联系租户管理员为你授权。”`
- 仪表盘整体不提供“提交申请”入口；不产生申请单与审批流转。

---

### 12.7 异常与降级（必须保证“局部坏不全坏”）

#### 12.7.1 数据集状态异常

- 数据集 `FAILED/BLOCKED/PAUSED` 时：

  - 相关图表展示“数据源不可用/数据可能过期”占位卡
  - 占位卡提供“查看数据集详情”入口（若用户有权限）
  - 仪表盘顶部可聚合提示“部分数据源不可用（n 个）”，点击展开列表

#### 12.7.2 图表配置失效

- 由于字段删除/类型不兼容导致图表渲染失败：

  - 占位卡提示“图表配置失效，需要编辑修复”
  - 若用户无 EDIT 权限：提示“联系仪表盘编辑者/管理员”

---

### 12.8 导出（仪表盘）

- **V1 固定：不支持整盘导出**（不提供“导出长图/导出 PDF/导出图片”入口）。
- V1 仅支持：
  - **单图表导出数据（CSV）**
  - **单图表导出图片（PNG）**
  - 规则与权限沿用第 11.9（导出前仍需满足数据集权限联合校验）。
...
- 创建/编辑/删除/授权/转移 Owner
- 记录操作者、时间、对象、结果（对齐你现有“操作审计”的记录方式风格 ）
...
1. **编辑闭环**：创建仪表盘 → 添加多个图表 → 调整布局 →（可选）配置局部标题/备注 → 保存 → 查看态展示一致。
2. **引用式更新**：图表 A 被加入仪表盘后，修改图表 A 的查询/可视化配置并保存 → 回到仪表盘点击刷新/重新打开 → 仪表盘展示按最新图表配置更新。
3. **联合权限校验（被动授权）**：仅有仪表盘 VIEW 权限但缺少数据集 VIEW 权限 → 进入仪表盘但相关图表占位提示正确且无申请入口；管理员授权后 → 用户刷新即可正常查看。
4. **数据集阻断降级**：数据集进入 `BLOCKED/FAILED/PAUSED` → 仪表盘仅相关图表降级，其他图表正常；占位卡“查看数据集详情”入口在用户有权限时可用。
5. **自动刷新**：开启自动刷新（1/5/15/60 分钟之一）→ 到点后能自动触发刷新；关闭后不再触发。
6. **导出限制**：仪表盘页面不出现整盘导出入口；单图表导出 CSV/PNG 仍可用且权限校验正确。

---

## 13. 审计日志模块（Audit Logs）

> 本章用于统一定义“操作审计/变更追踪”的最小可用实现，覆盖第 4.5.3、7.5、8.8、12.9 等章节中要求的审计能力。
...
- 为租户提供统一的审计日志入口，支持按时间、操作人、操作类型快速定位关键变更；
- 保证“权限变更”与“高风险结构变更（建模/报表资产）”可追溯；
- 后端保证记录完整、不可被普通用户篡改；前端提供只读查询与详情查看。

**本期纳入范围（必须）**

1. **权限变更审计**（对齐第 7.5）  
   角色、成员角色关系、资源权限、行权限、列权限的增删改。

2. **建模操作审计**（对齐第 8.8）  
   表/字段/关联字段的关键增删改（含失败尝试），以及必要的拦截类记录（如“被数据集引用无法删除字段”）。

3. **报表资产审计（数据集/图表/仪表盘）**（对齐第 10~12、尤其第 12.9）  
   数据集/图表/仪表盘的创建、编辑、删除、授权、Owner 转移、仪表盘布局调整、图表实例增删改。

**本期不做（Out-of-Scope）**

- 平台后台跨租户审计 UI（仅预留后端查询能力与权限模型，后续版本补齐）；
- 导出 CSV/Excel、订阅与告警、复杂统计报表；
- 对日志字段做细粒度脱敏配置（本期仅做最小必要的错误原因与变更摘要）。
...
- Settings → **审计日志（Audit Logs）**

**可见性（必须）**

- 租户 Owner：可查看本租户全部审计日志；
- Data Engineer：可只读查看（用于排查问题与追踪变更）；
- Analyst / Viewer：默认不可见。

> 上述默认策略与第 2.3 的模块访问矩阵保持一致；若后续要开放给自定义角色，本期以 “Settings 模块访问权限” 为前置门槛即可（不新增独立权限点）。

---

### 13.3 数据结构（后端）

统一新增（或复用）`AuditLog` 记录（逻辑表），用于承载所有模块的审计信息。

#### 13.3.1 核心字段（必须）

- `id`
- `tenant_id`（平台级日志可为空；本期租户日志必填）
- 操作人：

  - `actor_tenant_user_id`
  - `actor_global_user_id`
  - `actor_display_name`（快照，用于历史展示）
...
- 归类字段：
  ...
  - `module`：SETTINGS / MODELING / REPORTS
  - `action`：操作枚举（见 13.4）
  - `resource_type`：TABLE / FIELD / FLOW / DATASET / CHART / DASHBOARD / ROLE / TENANT_USER / ROW_PERMISSION / COLUMN_PERMISSION 等
  - `resource_id`（可空：如批量操作或作用于目录/规则集合）
  - `resource_name_snapshot`（可选）

- 变更内容：

  - `change_summary`（1 行可读摘要）
  - `before_json`（可选，JSON）
  - `after_json`（可选，JSON）
  - `extra_json`（可选：如 folder_id、chart_instance_id、layout_hash、request_id 等）

- 结果：

  - `result`：SUCCESS / FAILED
  - `error_code`（可选）
  - `error_message`（可选，限制长度，避免泄露敏感数据）

#### 13.3.2 索引与性能（必须）

- `(tenant_id, created_at DESC)`：支撑默认按时间倒序查询；
- `(tenant_id, module, created_at DESC)`：支撑按模块筛选；
- `(tenant_id, actor_tenant_user_id, created_at DESC)`：支撑按操作人筛选。
...
- 资源权限（RolePermission）：
  ...
  - `UPSERT_ROLE_PERMISSION`（新增/修改）
  - `DELETE_ROLE_PERMISSION`

- 行权限（RowPermission）：

  - `CREATE_ROW_PERMISSION`
  - `UPDATE_ROW_PERMISSION`
  - `DELETE_ROW_PERMISSION`

- 列权限（ColumnPermission）：

  - `UPDATE_COLUMN_PERMISSION`
...
> 对于“失败尝试”（如删除字段被阻断），也必须记录 `result=FAILED` 与原因。

#### 13.4.3 报表资产（对齐第 10~12、尤其第 12.9）
...
- 数据集：
  ...
  - `TRIGGER_DATASET_REFRESH`（如本期已支持手动刷新）
...
- 仪表盘：
  ...
  - `UPDATE_DASHBOARD`
  - `DELETE_DASHBOARD`
  - `TRANSFER_DASHBOARD_OWNER`
  - `GRANT_DASHBOARD_PERMISSION`
  - `REVOKE_DASHBOARD_PERMISSION`
...
### 13.5 查询列表页（Settings → Audit Logs）

#### 13.5.1 列表字段（必须）

默认列表按 `created_at DESC` 排序，字段：

| 字段         | 说明 |
| ------------ | ---- |
| 时间         | created_at |
| 操作人       | actor_display_name |
| 模块         | module |
| 操作类型     | action |
| 操作对象     | resource_type + resource_name_snapshot（无则展示 resource_id） |
| 结果         | SUCCESS / FAILED |

#### 13.5.2 筛选条件（必须）

- 时间范围（默认最近 7 天，可切换 24h / 7d / 30d / 自定义）
- 操作人（模糊搜索）
- 模块（SETTINGS / MODELING / REPORTS）
- 操作类型（action，多选）
- 结果（SUCCESS / FAILED）
...
- 基本信息：时间、操作人、模块、action、resource；
- 结果信息：SUCCESS/FAILED + error_message（若有）；
- 变更摘要：change_summary；
- 变更详情：before_json / after_json（折叠展示，支持复制 JSON）。
...
1. **权限变更可追溯**：创建角色 → 给角色配置表/数据集/仪表盘权限 → 审计日志可按操作人/时间筛选到记录，详情可见变更摘要。
