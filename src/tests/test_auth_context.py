...
class AuthContextAPITests(APISimpleTestCase):
    ...
    def test_protected_view_valid_token(self):
        ...
        response = self.client.get("/protected", HTTP_AUTHORIZATION=f"Bearer {token}")
        body = response.json()
    ...
    def test_admin_view_regular_user(self):
        ...
        token, _ = issue_access_token(user_id=100, is_platform_admin=False)
        response = self.client.get("/admin", HTTP_AUTHORIZATION=f"Bearer {token}")
        body = response.json()
    ...
    def test_protected_view_malformed_header(self):
        ...
        # 缺少 Bearer 前缀
        response = self.client.get("/protected", HTTP_AUTHORIZATION="invalid_token")
        body = response.json()
